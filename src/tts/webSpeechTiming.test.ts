...
import { HARD_HOLD_MAX_MS, SOFT_HOLD_MAX_MS, WebSpeechTimingClock, classifyGap } from './webSpeechTiming';
...
describe('text without punctuation', () => {
  it('keeps the original estimate and the plain re-anchor/free-run clock', () => {
    ...
    // Original layout: 72ms per character, 55ms between words.
    expect(h.result.words.map((word) => word.startMs)).toEqual([0, 415, 686, 957]);
